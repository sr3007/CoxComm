(function () {
    ...
    .directive('uiPeople', ['uiPeopleConfig', '$q', '$injector', '$resource', '$timeout', function (uiPeopleConfig, $q, $injector, $resource, $timeout) {
        ...
        return {
            ...
            scope: true,
            link: function (scope, elm, attrs, ngModelPP) {
                var rootScope = null;
                if (!rootScope) {
                ...
                var principalAccountType = (attrs.ppAccountType && attrs.ppAccountType.match(/^(?:user)|(?:dl)|(?:secgroup)|(?:spgroup)$/i)) ? attrs.ppAccountType : 'User,SPGroup';
                ...
                models.pickerSchema = function (accountType, multipleValues, width) {
                    ...
                    this.PrincipalAccountType = accountType ? accountType : 'User,SPGroup';
                ...
                var updateView = function () {
                    ngModelPP.$setViewValue(userModel.getViewValue());
                    if (!scope.$root.$$phase) {
                        scope.$apply();
                    }
                };
                var getViewValue = function () {
                    return ngModelPP.$modelValue;
                }
                ngModelPP.$render = Function.createDelegate(scope, function () {
                    scope.isDisabled = attrs.ngDisabled ? scope.$eval(attrs.ngDisabled) : false;
                    userModel.updateFromViewValue(ngModelPP.$modelValue, scope);
                    if (scope.isDisabled) {
